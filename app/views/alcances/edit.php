...
<div class="row">
    <div class="col-md-8 mx-auto">
        <div class="card card-body bg-light mt-5">
            ...
            <p>Edite los detalles del alcance (ID: <?php echo $data['id']; ?>).</p>
            <form action="<?php echo URLROOT; ?>/alcances/edit/<?php echo $data['id']; ?>" method="post">
                    ...
                    >
                        ...
                        if (isset($data['contratos']) && is_array($data['contratos'])) :
                            foreach($data['contratos'] as $contrato): 
                                ...
                                $contrato_display = 'ID ' . $contrato->Id_contrato . ' - ' . substr($contrato->Descripcion, 0, 50) . '...';
                            ?>
                                <option 
                                    value="<?php echo $contrato->Id_contrato; ?>" 
                                    <?php 
                                    // Compara el ID del contrato actual del formulario con el ID del contrato en el bucle
                                    echo ($data['id_contrato'] == $contrato->Id_contrato) ? 'selected' : ''; 
                                    ?>
                                >
